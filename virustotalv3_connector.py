...
class VirustotalV3Connector(BaseConnector):
    ...
    def _process_json_response(self, r, action_result):
        ...
        if error_info.get("code") and error_info.get("message"):
            error_details = {"message": error_info.get("code"), "detail": error_info.get("message")}
            return RetVal(
                action_result.set_status(phantom.APP_ERROR, f"Error from server, Status Code: {r.status_code} data returned: {error_details}"),
                resp_json,
            )
        else:
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR,
                    "Error from server, Status Code: {} data returned: {}".format(r.status_code, r.text.replace("{", "{{").replace("}", "}}")),
                ),
                resp_json,
            )
    ...
    def _make_rest_call(self, action_result, endpoint, params=None, body=None, headers=None, files=None, method="get", large_file=False):
        ...
        if self._reputation_cache_length and self._reputation_cache_interval and call.endswith("_reputation"):
            ...
            if entry:
                self.debug_print(f"Key {cache_key} retrieved from cache")
        ...
        try:
            response = request_func(url, params=params, data=body, headers=headers, files=files, verify=self._verify_ssl, timeout=self._timeout)
        except Exception as e:
            # Set the action_result status to error, the handler function will most probably return as is
            error_message = self._get_error_message_from_exception(e)
        ...
        if cache and cache_key:
            ...
            self.debug_print(f"Key {cache_key} saved to cache")
    ...
    def _save_file_to_vault(self, action_result, response, file_hash):
        ...
        status, vault_ret_message, vault_id = ph_rules.vault_add(
            file_location=file_path, container=self.get_container_id(), file_name=file_name, metadata={"contains": contains}
        )
    ...
    def _handle_domain_reputation(self, param):
        ...
        if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
            return self.virustotalv3_action_result.set_status(
                phantom.APP_SUCCESS,
                VIRUSTOTAL_SUCCESS_MSG_WITH_ERROR.format(
                    object_name=object_name, object_value=object_value, error_code=json_resp["error"]["code"]
                ),
            )

        if "data" not in json_resp:
            return self.virustotalv3_action_result.set_status(
                phantom.APP_ERROR, VIRUSTOTAL_ERROR_MSG_OBJECT_QUERIED.format(object_name=object_name, object_value=object_value)
            )
    ...
    def _handle_file_reputation(self, param):
        ...
        ret_val, json_resp = self._make_rest_call(
            self.virustotalv3_action_result, FILE_REPUTATION_ENDPOINT.format(id=hash), headers=self._headers
        )
        if phantom.is_fail(ret_val):
            return ret_val
        ...
        if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
            return self.virustotalv3_action_result.set_status(
                phantom.APP_SUCCESS,
                VIRUSTOTAL_SUCCESS_MSG_WITH_ERROR.format(object_name="hash", object_value=param["hash"], error_code=json_resp["error"]["code"]),
            )

        if "data" not in json_resp:
            return self.virustotalv3_action_result.set_status(
                phantom.APP_ERROR, VIRUSTOTAL_ERROR_MSG_OBJECT_QUERIED.format(object_name="Hash", object_value=hash)
            )
    ...
    def _handle_get_file(self, param):
        ...
        query_url = f"{BASE_URL}{GET_FILE_API_ENDPOINT.format(id=file_hash)}"
        ...
        if r.status_code != 200:
            return self.virustotalv3_action_result.set_status(
                phantom.APP_ERROR, VIRUSTOTAL_SERVER_RETURNED_ERROR_CODE.format(code=r.status_code)
            )
    ...
    def _handle_url_reputation(self, param):
        ...
        item_summary = self.virustotalv3_action_result.set_summary({})
        ret_val, json_resp = self._make_rest_call(
            self.virustotalv3_action_result, URL_REPUTATION_ENDPOINT.format(id=url_id), headers=self._headers, method="get"
        )

        if phantom.is_fail(ret_val):
            return self.virustotalv3_action_result.set_status(
                phantom.APP_ERROR, self.virustotalv3_action_result.get_message().replace(url_id, url)
            )

        if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
            return self.virustotalv3_action_result.set_status(
                phantom.APP_SUCCESS,
                VIRUSTOTAL_SUCCESS_MSG_WITH_ERROR.format(object_name="URL", object_value=param["url"], error_code=json_resp["error"]["code"]),
            )

        if "data" not in json_resp:
            return self.virustotalv3_action_result.set_status(
                phantom.APP_ERROR, VIRUSTOTAL_ERROR_MSG_OBJECT_QUERIED.format(object_name="URL", object_value=param["url"])
            )
    ...
    def _handle_detonate_url(self, param):
        ...
        ret_val, wait_time = self._validate_integers(
            self.virustotalv3_action_result, param.get("wait_time", self._wait_time), "wait_time", allow_zero=True
        )
        ...
        if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
            ret_val, json_resp = self._make_rest_call(
                self.virustotalv3_action_result, URL_API_ENDPOINT, body=data, headers=self._headers, method="post"
            )
            if phantom.is_fail(ret_val):
                return ret_val
    ...
    def _handle_detonate_file(self, param):
        ...
        try:
            ...
            if VIRUSTOTAL_EXPECTED_ERROR_MSG in error_message:
                ...
                return self.virustotalv3_action_result.set_status(phantom.APP_ERROR, f"Unable to retrieve file from vault: {error_message}")

        file_hash = file_info["metadata"]["sha256"]

        # check if report already exists
        ret_val, json_resp = self._make_rest_call(
            self.virustotalv3_action_result, FILE_REPUTATION_ENDPOINT.format(id=file_hash), headers=self._headers
        )
        ...
        if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                return self.virustotalv3_action_result.set_status(phantom.APP_ERROR, f"Error occurred while reading file. {error_message}")
            ...
            if (file_size / 1000000) > 32:
                ...
                ret_val, json_resp = self._make_rest_call(
                    self.virustotalv3_action_result, upload_url, headers=self._headers, files=files, method="post", large_file=True
                )

            else:
                ret_val, json_resp = self._make_rest_call(
                    self.virustotalv3_action_result, FILE_REPORT_ENDPOINT, headers=self._headers, files=files, method="post"
                )
        ...
        if "data" not in json_resp:
            return self.virustotalv3_action_result.set_status(
                phantom.APP_ERROR, VIRUSTOTAL_ERROR_MSG_OBJECT_QUERIED.format(object_name="Hash", object_value=file_hash)
            )
    ...
    def _handle_get_cached_entries(self):
        ...
        self.virustotalv3_action_result.update_summary(
            {"count": len(data), "expiration_interval": self._reputation_cache_interval, "max_cache_length": self._reputation_cache_length}
        )
    ...
    def _poll_for_result(self, action_result, scan_id, poll_interval, wait_time):
        ...
        while True:
            ...
            if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
                return self.virustotalv3_action_result.set_status(
                    phantom.APP_SUCCESS,
                    VIRUSTOTAL_SUCCESS_MSG_WITH_ERROR.format(object_name="Scan ID", object_value=scan_id, error_code=json_resp["error"]["code"]),
                )
    ...
    def _handle_get_quotas(self, param):
        ...
        item_summary = self.virustotalv3_action_result.set_summary({})
        ret_val, json_resp = self._make_rest_call(
            self.virustotalv3_action_result, QUOTA_ENDPOINT.format(id=user_id), headers=self._headers, method="get"
        )
        ...
        if json_resp.get("error", {}).get("code") in PASS_ERROR_CODE.values():
            return self.virustotalv3_action_result.set_status(
                phantom.APP_SUCCESS,
                VIRUSTOTAL_SUCCESS_MSG_WITH_ERROR.format(
                    object_name="User ID", object_value=param["user_id"], error_code=json_resp["error"]["code"]
                ),
            )
    ...
    def handle_action(self, param):
        ...
        self.virustotalv3_action_result._ActionResult__data = json.loads(
            json.dumps(self.virustotalv3_action_result._ActionResult__data).replace("\\u0000", "\\\\u0000")
        )
    ...
    def initialize(self):
        ...
        try:
            ...
            return self._initialize_error(
                "Rate Limit asset setting not configured! Please validate asset configuration and save", Exception(f"KeyError: {ke}")
            )
