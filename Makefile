...
# Pipeline secrets
SECRETS = app_artf_token mc_artf_token gitlab_api_token app_deploy_key
ifneq ($(wildcard /run/secrets/.),)
 ...
 export MC_ARTF_TOKEN     ?= $(shell cat /run/secrets/mc_artf_token)
 export APP_DEPLOY_KEY    ?= $(shell cat /run/secrets/app_deploy_key)
endif

APP_RELEASE_TARGETS = test upload build release deploy
